...
function getVineItemUrl(product) {
  if (product.recommendationId) {
    const vineData = JSON.stringify({
      asin:             product.asin,
      isParentAsin:     false,
      recommendationId: product.recommendationId
    });
    return `https://www.amazon.com/vine/vine-items?vine-data=${encodeURIComponent(vineData)}`;
  }
  return getVineSearchUrl(product.asin);
}
